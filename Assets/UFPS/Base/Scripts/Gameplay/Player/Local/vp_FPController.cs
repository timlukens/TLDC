...
public class vp_FPController : vp_CharacterController
{
    ...
    protected Vector3 m_InitialJumpForce = Vector3.zero;
    protected Vector3 m_JumpingForward = Vector3.zero;
    protected float m_LastRotationY = 0.0f;
	...
	protected override void Start()
	{
		...
		if (PhysicsHasCollisionTrigger)
		{
            ...
            m_LastRotationY = transform.localEulerAngles.y;
	...
	protected override void FixedUpdate()
	{
        ...
        if(m_Grounded) gameObject.GetComponent<vp_FPInput>().FPPlayer.Jump.Stop();
    ...
    protected virtual void UpdateThrottleWalk()
    {
        ...
        //foward
        //if grounded we want to move in the forward direction
        if (m_Grounded)
        {
            m_MotorThrottle +=
        ...
        //otherwise we want to just continue the original forward direction, not the current forward direction
        else
        {
            //m_MotorThrottle.z 
//            m_MotorThrottle +=
...
//                    m_MotorAirSpeedModifier) *
//                    m_SlopeFactor *
//                    (Transform.InverseTransformDirection(m_Velocity).z * 0.01f);
        }
        ...
        //air strafe
        if(Player.InputMoveVector.Get().x != 0 && Player.InputMoveVector.Get().y == 0 && !m_Grounded)
        {
            float currentYRot = transform.localEulerAngles.y;
            if (m_LastRotationY >= 270 && currentYRot < 90) m_LastRotationY -= 360.0f;
            else if (currentYRot >= 270 && m_LastRotationY < 90) currentYRot -= 360.0f;
            float distance = currentYRot - m_LastRotationY;

            if (distance > 0 && Player.InputMoveVector.Get().x < 0) distance = 0.0f;
            if (distance < 0 && Player.InputMoveVector.Get().x > 0) distance = 0.0f;

            //distance *= 0.5f;
            distance = Mathf.Abs(distance);
            distance += Transform.InverseTransformDirection(m_Velocity).z * 0.25f;// * 0.1f;
            if (distance > 1) distance = 1;

            //Debug.Log(Transform.InverseTransformDirection(m_Velocity));

            //m_MotorThrottle.z += Player.InputMoveVector.Get().x * 0.1f;
            //float z = Transform.InverseTransformDirection(m_Velocity).z;

            //Debug.Log(z);
            m_MotorThrottle += distance * (Transform.TransformDirection(
                Vector3.forward *
                (MotorAcceleration * 0.01f) *
                m_MotorAirSpeedModifier) *
                m_SlopeFactor);

            m_JumpingForward = Transform.TransformDirection(Vector3.forward);
        }
        ...
        // dampen motor force
		if (m_Grounded)// || (Player.InputMoveVector.Get().x != 0 || Player.InputMoveVector.Get().y != 0))
        {
	...
	protected virtual void UpdateJumpForceWalk()
	{
		...
		if (Player.Jump.Active)
		{
            ...
            {
                m_InitialJumpForce = new Vector3(m_MotorThrottle.x, 0, m_MotorThrottle.z);
            }
		}
